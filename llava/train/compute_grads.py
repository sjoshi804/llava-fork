...
def preprocess_mpt(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if i != 0 and getattr(tokenizer, 'levegacy', False) and IS_TOKENIZER_GREATER_THAN_0_14:
                round_len += 1
                instruction_len += 1
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        with open(data_path, "r") as f:
            data = json.load(f)
        list_data_dict = data["samples"]
        self.list_data_dict = list_data_dict
        self.data_args.image_folder = data["image_folder"]
        rank0_print("Formatting inputs...Skip in lazy mode")
        rank0_print(f"Found {len(self.list_data_dict)} samples.")
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            img_tokens = 128 if "image_1" in sample else 0
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = cur_len if "image_1" in sample else -cur_len
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image_1" in sources[0]:
            image_file = self.list_data_dict[i]['image_1']
        ...
        data_dict = preprocess(
            sources,
            self.tokenizer,
            has_image=("image_1" in self.list_data_dict[i]))
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0],
                             labels=data_dict["labels"][0])

        # image exist in the data
        if "image_1" in self.list_data_dict[i]:
            data_dict["image_1"] = image
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict["image_1"] = torch.zeros(3, crop_size['height'], crop_size['width'])
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image_1" in instances[0]:
            images = [instance["image_1"] for instance in instances]
...
def compute_grad(attn_implementation=None):
    global local_rank
